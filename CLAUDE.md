...
**Cross-tab shared inputs.** Character / Style / Story are synced across the three generation pages via `shared_inputs.js` (a non-module loaded before each page's script, exposing `window.SharedInputs` with `read()`/`patch()`/`onExternalChange()`), backed by `localStorage['monkeyking_shared_inputs']`. Book Builder writes immediately on input (no debounce) so values aren't lost on navigation; the other pages debounce. On Book Builder load, if a saved book (`monkeyking_bb_state`) conflicts with newer shared inputs edited in another tab, it **asks the user** whether to continue the saved book or start fresh from the synced inputs (`sharedConflictsWithSaved` + `clearProject({ keepInputs })`).
